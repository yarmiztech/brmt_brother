...
class BranchExpenses(models.Model):
    ...
    def post_expense(self):
        ...
        if synch:
            ...
            upd = models.execute_kw(db, uid, password, 'branch.expenses', 'write', [[estimate[0]['id']],
                                                                                    {
                                                                                        'basic_synch_post_expense_button': True,
                                                                                    }])
        return super(BranchExpenses, self).post_expense()
    ...
    def constraints_curreny(self):
        ...
        if synch:
            for each in self:
                ...
                req = self.env['vehicle.request'].search([('name', '=', each.vehicle_req.name)])
                ...
                vehicle_ser = models.execute_kw(db, uid, password, 'fleet.vehicle', 'search_read',
                                                [[['license_plate', '=', each.vehicle_id.license_plate]]],
                                                {'fields': ['name', 'id']})
                if vehicle_ser:
                    vehicle_ser = vehicle_ser[0]['id']
                else:
    ...
    def constraints_freight(self):
        ...
        if synch:
            for each in self:
                ...
                vehicle_ser = models.execute_kw(db, uid, password, 'fleet.vehicle', 'search_read',
                                                [[['license_plate', '=', each.vehicle_id[0].license_plate]]],
                                                {'fields': ['name', 'id']})
                if vehicle_ser:
                    vehicle_ser = vehicle_ser[0]['id']
                else:
